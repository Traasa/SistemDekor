...
<body>
    ...
    <div class="summary-box">
        ...
        <div class="summary-row">
            ...
            <span><strong>{{ $data['total_events'] }}</strong></span>
        </div>
        <div class="summary-row">
            <span>Total Revenue</span>
            <span><strong>Rp {{ number_format($data['total_revenue'], 0, ',', '.') }}</strong></span>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ ucfirst($detail['event_type']) }}</td>
                <td class="text-right">Rp {{ number_format($detail['total_price'], 0, ',', '.') }}</td>
                <td class="text-right">Rp {{ number_format($detail['total_paid'], 0, ',', '.') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    @else
    <p style="text-align: center; padding: 20px; color: #999;">Tidak ada data event</p>
    @endif

    <div class="footer">
        <p>Wedding Organizer - Laporan Event</p>
