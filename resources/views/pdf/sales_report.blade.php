    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($orders as $order)
                ...
                <tr>
                    ...
                    <td>{{ $order->order_number ?? $order->order_code }}</td>
                    <td>{{ $order->client->name ?? 'N/A' }}</td>
                    <td class="text-right">{{ number_format($order->total_price, 2) }}</td>
                    <td class="text-right">{{ number_format($order->discount, 2) }}</td>
                    <td class="text-right">{{ number_format($order->additional_costs ?? 0, 2) }}</td>
                    <td class="text-right">{{ number_format($order->final_price, 2) }}</td>
                    <td class="text-right">{{ number_format($pembayaran, 2) }}</td>
        ...
        <tfoot>
            <tr class="total-row">
                ...
                <td class="text-right">{{ number_format($sumDiskon, 2) }}</td>
                <td class="text-right">{{ number_format($sumPajak, 2) }}</td>
                <td class="text-right">{{ number_format($sumTotal, 2) }}</td>
                <td class="text-right">{{ number_format($sumPembayaran, 2) }}</td>
