...
<head>
    <meta charset="UTF-8">
    <title>Kontrak {{ $order->order_number ?? ('ORD-' . $order->id) }}</title>
    <style>
...
<body>
    ...
    <div class="doc-number">Nomor Kontrak: <strong>{{ $order->order_number ?? ('ORD-' . $order->id) }}</strong></div>

    <p>Pada hari ini, <strong>{{ optional($order->created_at)->format('d M Y') ?? '-' }}</strong>, yang bertandatangan di bawah ini secara sadar dan tanpa paksaan dari pihak mana pun, menyepakati ikatan kerja sama antara:</p>
    ...
    <table class="data-table">
        <tr>
            ...
            <td class="colon">:</td>
            <td class="value">{{ $order->client->name ?? '-' }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="colon">:</td>
            <td class="value">{{ $order->client->phone ?? '-' }}</td>
        </tr>
        <tr>
            <td class="label">Alamat Lengkap</td>
            <td class="colon">:</td>
            <td class="value">{{ $order->client->address ?? '-' }}</td>
        </tr>
    </table>
    ...
    <p>PIHAK PERTAMA berkewajiban untuk menyediakan, mempersiapkan, dan melaksanakan layanan Wedding Organizer & Decoration sesuai dengan spesifikasi paket <strong>"Paket {{ $order->package->name ?? 'Custom Package' }}"</strong> yang telah dipilih dan disetujui oleh PIHAK KEDUA.</p>
    ...
    <table class="data-table">
        <tr>
            ...
            <td class="colon">:</td>
            <td class="value">{{ optional($order->event_date)->format('d M Y') ?? '-' }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="colon">:</td>
            <td class="value">{{ $order->event_location ?? $order->event_address ?? '-' }}</td>
        </tr>
    </table>

    <div class="section-title">PASAL 3: NILAI KONTRAK & SKEMA PEMBAYARAN</div>
    <ol>
        <li>Total nilai kontrak investasi pernikahan yang disepakati oleh kedua belah pihak adalah sebesar <strong>Rp {{ number_format($order->final_price ?? 0, 0, ',', '.') }}</strong>.</li>
    ...
    <ol>
        <li>PIHAK KEDUA berhak mendapatkan seluruh rincian fasilitas serta pelayanan profesional dari paket "Paket {{ $order->package->name ?? 'Custom Package' }}".</li>
    ...
    <table class="signature-container">
        <tr>
            ...
            <td>
                ...
                <div class="signature-space"></div>
                <div class="signature-line">{{ $order->client->name ?? '__________________' }}</div>
