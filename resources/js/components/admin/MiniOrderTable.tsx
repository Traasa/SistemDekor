...
import { MiniOrder } from '../../services/apiService';

interface MiniOrderTableProps {
    orders: MiniOrder[];
    isLoading: boolean;
...
export const MiniOrderTable: React.FC<MiniOrderTableProps> = ({ orders, isLoading, onStatusChange, onDelete, onViewDetail }) => {
    ...
    if (orders.length === 0) {
        return (
            <div className="rounded-xl bg-white p-8 text-center shadow-sm">
                ...
                <p className="text-gray-600">Belum ada mini order yang terdaftar atau tidak ditemukan.</p>
            </div>
        );
    }
                ...
                <table className="w-full">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Vendor Client</th>
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                        {orders.map((order) => (
                            <tr key={order.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="text-sm text-gray-900">{order.vendor_client?.name || 'N/A'}</div>
                                    <div className="text-sm text-gray-500">{order.vendor_client?.company_name || ''}</div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="text-sm text-gray-900">{order.event_name}</div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="text-sm font-semibold text-gray-900">{formatCurrency(Number(order.final_price || 0))}</div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <select
                                        value={order.status}
